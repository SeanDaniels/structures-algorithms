...
public:
  ...
  vector<int> visitedTable, unvisited, visitedSet, backEdges;
  ...
  void addEdge(int src, int dst) { adjacencyList[src][dst] = 1; }
  ...
  bool isVisited(int node) {
    ...
    if (found != visitedSet.end())
      return true;
    return false;
  }

  bool isBackEdge(int node) {
    auto it = find(backEdges.begin(), backEdges.end(), node);
    if (it != backEdges.end())
      return true;
    return false;
  }
  ...
  void dfs_a(int targetNode) {
    ...
    if (isVisited(targetNode)) {
      ...
      // add edges of current node to unvisited list (assume nodes aren't
      // visited)
      for (int i = 0; i < numberOfNodes; i++) {
        if (tempAdjacencyList[i] != 0)
